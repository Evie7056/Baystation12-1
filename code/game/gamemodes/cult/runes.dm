...
/obj/effect/rune/tearreality/cast(var/mob/living/user)
	...
	var/area/A = get_area(src)
	command_announcement.Announce("Высокий уровень подпространственных помех обнаружен в отсеке '[A]'. Подозревается образование червоточины. Немедленно расследуйте данное происшествие.")
	...
	if(the_end_comes >= the_time_has_come)
		...
		command_announcement.Announce("Проявление подпространственной аномалии прекратились. Работа устройств подпространственной связи успешно восстановлена.")
		qdel(src)
